// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "Engine/Public/ReplayNetConnection.h"
#include "Serialization/ArchiveUObjectFromStructuredArchive.h"
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable : 4883)
#endif
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeReplayNetConnection() {}
// Cross Module References
	ENGINE_API UClass* Z_Construct_UClass_UReplayNetConnection_NoRegister();
	ENGINE_API UClass* Z_Construct_UClass_UReplayNetConnection();
	ENGINE_API UClass* Z_Construct_UClass_UNetConnection();
	UPackage* Z_Construct_UPackage__Script_Engine();
// End Cross Module References
	void UReplayNetConnection::StaticRegisterNativesUReplayNetConnection()
	{
	}
	UClass* Z_Construct_UClass_UReplayNetConnection_NoRegister()
	{
		return UReplayNetConnection::StaticClass();
	}
	struct Z_Construct_UClass_UReplayNetConnection_Statics
	{
		static UObject* (*const DependentSingletons[])();
#if WITH_METADATA
		static const UE4CodeGen_Private::FMetaDataPairParam Class_MetaDataParams[];
#endif
		static const FCppClassTypeInfoStatic StaticCppClassTypeInfo;
		static const UE4CodeGen_Private::FClassParams ClassParams;
	};
	UObject* (*const Z_Construct_UClass_UReplayNetConnection_Statics::DependentSingletons[])() = {
		(UObject* (*)())Z_Construct_UClass_UNetConnection,
		(UObject* (*)())Z_Construct_UPackage__Script_Engine,
	};
#if WITH_METADATA
	const UE4CodeGen_Private::FMetaDataPairParam Z_Construct_UClass_UReplayNetConnection_Statics::Class_MetaDataParams[] = {
		{ "IncludePath", "ReplayNetConnection.h" },
		{ "ModuleRelativePath", "Public/ReplayNetConnection.h" },
		{ "ObjectInitializerConstructorDeclared", "" },
	};
#endif
	const FCppClassTypeInfoStatic Z_Construct_UClass_UReplayNetConnection_Statics::StaticCppClassTypeInfo = {
		TCppClassTypeTraits<UReplayNetConnection>::IsAbstract,
	};
	const UE4CodeGen_Private::FClassParams Z_Construct_UClass_UReplayNetConnection_Statics::ClassParams = {
		&UReplayNetConnection::StaticClass,
		"Engine",
		&StaticCppClassTypeInfo,
		DependentSingletons,
		nullptr,
		nullptr,
		nullptr,
		UE_ARRAY_COUNT(DependentSingletons),
		0,
		0,
		0,
		0x001000ACu,
		METADATA_PARAMS(Z_Construct_UClass_UReplayNetConnection_Statics::Class_MetaDataParams, UE_ARRAY_COUNT(Z_Construct_UClass_UReplayNetConnection_Statics::Class_MetaDataParams))
	};
	UClass* Z_Construct_UClass_UReplayNetConnection()
	{
		static UClass* OuterClass = nullptr;
		if (!OuterClass)
		{
			UE4CodeGen_Private::ConstructUClass(OuterClass, Z_Construct_UClass_UReplayNetConnection_Statics::ClassParams);
		}
		return OuterClass;
	}
	IMPLEMENT_CLASS(UReplayNetConnection, 4244428944);
	template<> ENGINE_API UClass* StaticClass<UReplayNetConnection>()
	{
		return UReplayNetConnection::StaticClass();
	}
	static FCompiledInDefer Z_CompiledInDefer_UClass_UReplayNetConnection(Z_Construct_UClass_UReplayNetConnection, &UReplayNetConnection::StaticClass, TEXT("/Script/Engine"), TEXT("UReplayNetConnection"), false, nullptr, nullptr, nullptr);
	DEFINE_VTABLE_PTR_HELPER_CTOR(UReplayNetConnection);
	IMPLEMENT_FSTRUCTUREDARCHIVE_SERIALIZER(UReplayNetConnection)
PRAGMA_ENABLE_DEPRECATION_WARNINGS
#ifdef _MSC_VER
#pragma warning (pop)
#endif
